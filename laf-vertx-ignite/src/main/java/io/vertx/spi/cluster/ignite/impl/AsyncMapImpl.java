...
public class AsyncMapImpl<K, V> implements AsyncMap<K, V> {
    ...
    @Override
    public void get(final K key, final Handler<AsyncResult<V>> handler) {
        execute(cache -> cache.getAsync(marshal(key)), handler);
    }

    @Override
    public void put(final K key, final V value, final Handler<AsyncResult<Void>> handler) {
        execute(cache -> cache.putAsync(marshal(key), marshal(value)), handler);
    }

    @Override
    public void put(final K key, final V value, final long ttl, final Handler<AsyncResult<Void>> handler) {
        executeWithTtl(cache -> cache.putAsync(marshal(key), marshal(value)), handler, ttl);
    }

    @Override
    public void putIfAbsent(final K key, final V value, final Handler<AsyncResult<V>> handler) {
        execute(cache -> cache.getAndPutIfAbsentAsync(marshal(key), marshal(value)), handler);
    }

    @Override
    public void putIfAbsent(final K key, final V value, final long ttl, final Handler<AsyncResult<V>> handler) {
        executeWithTtl(cache -> cache.getAndPutIfAbsentAsync(marshal(key), marshal(value)), handler, ttl);
    }

    @Override
    public void remove(final K key, final Handler<AsyncResult<V>> handler) {
        execute(cache -> cache.getAndRemoveAsync(marshal(key)), handler);
    }

    @Override
    public void removeIfPresent(final K key, final V value, final Handler<AsyncResult<Boolean>> handler) {
        execute(cache -> cache.removeAsync(marshal(key), marshal(value)), handler);
    }

    @Override
    public void replace(final K key, final V value, final Handler<AsyncResult<V>> handler) {
        execute(cache -> cache.getAndReplaceAsync(marshal(key), marshal(value)), handler);
    }

    @Override
    public void replaceIfPresent(final K key, final V oldValue, final V newValue, final Handler<AsyncResult<Boolean>> handler) {
    ...
    @Override
    public void values(final Handler<AsyncResult<List<V>>> resultHandler) {
        Future<Map<K, V>> entriesFuture = Future.future();
        entries(entriesFuture);
    ...
    @Override
    public void entries(final Handler<AsyncResult<Map<K, V>>> resultHandler) {
    ...
    protected <T> void execute(final Function<IgniteCache<K, V>, IgniteFuture<T>> cacheOp,
     ...
     * @param <T>
     */
    protected <T> void executeWithTtl(final Function<IgniteCache<K, V>, IgniteFuture<T>> cacheOp,
                                      final Handler<AsyncResult<T>> handler, final long ttl) {
        try {
